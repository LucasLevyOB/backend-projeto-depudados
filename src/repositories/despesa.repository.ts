...
export class DespesaRepository {
  ...
  async getGastosDespesasByDeputado(idDeputado: number): Promise<number> {
    const despesas = await Despesa.find({ idDeputado: idDeputado })
      .select("valorLiquido")
      .lean();
  ...
  async getResumoGastosByDeputado(idDeputado: number): Promise<any[]> {
    const despesas = await Despesa.find({ idDeputado: idDeputado })
      .select("ano mes valorLiquido descricao")
      .lean();
    ...
    for (const d of despesas) {
      ...
      if (!mapaAnos.has(d.ano)) {
        mapaAnos.set(d.ano, {
          ano: d.ano,
          totalGastos: 0,
          mesesMap: new Map<number, number>(),
          categoriasMap: new Map<string, number>(),
        });
      }
    ...
    return Array.from(mapaAnos.values())
      .map((anoData) => {
        const mesesArray = Array.from(anoData.mesesMap.entries())
          .map(([mes, totalGasto]) => ({
            mes,
        ...
        const categoriasArray = Array.from(anoData.categoriasMap.entries())
          .map(([descricao, totalGasto]) => ({
            descricao,
            totalGasto: Number(totalGasto.toFixed(2)),
          }))
